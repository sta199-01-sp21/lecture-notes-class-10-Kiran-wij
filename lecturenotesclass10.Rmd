...
168 total patients were enrolled in the trial

- What is the probability a randomly selected patient received the placebo?
The probability a randomly selected patient recieved the placebo is is 0.256

- What is the probability a randomly selected patient received the placebo
  and had a marked improvement?
  The probability of this is 0.0417
...
- What is the probability a randomly selected patient had some improvement or
  ...
  The probability of this is 0.369
...
How about white?
The probability that the marble is blue is 3/8. The probability that it is white
is 1/8.

We can simulate this "drawing" with the `sample()` function.

```{r sample}
#set.seed(100) to get same random draw every time
...
This is incredibly close to the true probability values for all clours. In fact,
for white marbles it is precisely the true probability.
...
I think there will be a higher probability that absolute value of the difference 
between the two dice rolls is 4 because there are four different ways that I
think this could happen [(1,5), (2,6), (5,1), (6,2)] versus just two ways in
which the second roll could be a square of the first.
...
two_dice_roll <- two_dice_roll %>%
  mutate(square = die_1^2 == die_2) %>% 
  mutate(die_diff = abs(die_1 - die_2) == 4)
```

```{r summarize_rolls}
two_dice_roll %>% 
  summarize(prop_square = mean(square), prop_diff = mean(die_diff))
```

With our simmulation we got prop_square = 0.0567 and prop_diff = 0.114.
